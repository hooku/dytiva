...
#define MAX_MCU_TEMP_SAMPLE     20
    ...
    while (current_state == S_TEMP)
    {
        ...
        lcd_printf(0, 1, "TMP75=%.2fC ", degree_lm75);
        lcd_printf(0, 2, "MCU  =%.2fC ", degree_mcu);
        m4_dbgprt("Temp=%.1fC\r\n", degree_lm75);
        //sleep(100);
        ani(100);
        ani(100);
...
#define PWM_DELTA           1
    ...
    while (current_state == S_PWM)
    {
        ...
        pwm_set(pwm_percent);
        lcd_printf(0, 1, "PWM=%d  ", pwm_percent);
...
void demo_adc_photo(void)
{
    ...
    while (current_state == S_ADC_PHOTO)
    {
        ...
        if (adc0_value > (ADC_RESOLUTION/6*5))
        {
            gpio_led_all(1);
...
void demo_accel(void)
{
    int32_t raw_accel_x, raw_accel_y, raw_accel_z;
    float accel_x, accel_y, accel_z;
    
    int8_t i2cReadBuffer[6];
...
//        accel_z = raw_accel_z;
        ...
        lcd_printf(0, 1, "X=%d   ", raw_accel_x);
        lcd_printf(0, 2, "Y=%d   ", raw_accel_y);
        lcd_printf(0, 3, "Z=%d   ", raw_accel_z);
#endif
