...
void lcd_printf(uint8_t x, uint8_t y, const char *format, ...)
{
    ...
    for (i_msg = 0; i_msg < msg_len; i_msg ++)
    {
        ...
        {
            // we need check ascii range to prevent address violation
            if (isprint(msg_buffer[i_msg]))
            {
...
void lcd_draw(uint8_t x, uint8_t y, uint8_t width, uint8_t height, uint8_t *buffer)
{
    // should implemet condition x>16 y>16
    UC1701FontDisplay(y, x, width, height, buffer);
}
