...
uint8_t TMP100AddrTest(uint8_t addr)
{
    uint32_t i2cErrorState;

    I2CMasterSlaveAddrSet(TMP_PIN_I2C_PORT, addr, false);
    I2CMasterDataPut(TMP_PIN_I2C_PORT, 0x00);
    
    I2CMasterControl(TMP_PIN_I2C_PORT, I2C_MASTER_CMD_BURST_SEND_START);

    SysCtlDelay(500000);

    i2cErrorState = I2CMasterErr(TMP_PIN_I2C_PORT);

    if(i2cErrorState & I2C_MASTER_ERR_ADDR_ACK)
    {
        // device at selected address did not acknowledge
        return 0;
    }
